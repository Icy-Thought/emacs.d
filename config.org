...
(use-feature newsticker
  ...
  (defun irkalla/newsticker-quit-newTab ()
    ...
    (newsticker-stop)
    (tab-close))
  :hook (newsticker-treeview-item-mode . visual-line-fill-column-mode)
...
(use-package ement
  ...
  :hook (ement-room-mode . visual-line-fill-column-mode)
...
(use-feature telega ;; <-^ fetched from Nixpkgs
  :commands (telega)
  :hook (telega-chat-mode . visual-line-fill-column-mode)
  :custom
...
(use-feature flymake
  ...
  :config
  <<flymake-fringe-override>>
  <<flymake-fringe-symbols>>)
#+end_src

***** Replacing Bitmaps With Nerd Icons
:PROPERTIES:
:HEADER-ARGS: :tangle no
:END:

- Source: https://www.reddit.com/r/emacs/comments/1bawdau/making_flymake_supports_error_indicators_in_margin/

#+NAME: flymake-fringe-override
#+begin_src emacs-lisp
(advice-add #'flymake--fringe-overlay-spec :override
   (lambda (bitmap &optional recursed)
     (if (and (symbolp bitmap)
              (boundp bitmap)
              (not recursed))
         (flymake--fringe-overlay-spec (symbol-value bitmap) t)
       (and flymake-fringe-indicator-position
            bitmap (concat
                    (propertize "!" 'display `((margin left-margin) ,bitmap))
                    (propertize " " 'face    '(:inherit default :underline nil :stipple nil)
                                    'display `((margin left-margin) (space :width 5))))))))
#+end_src

Now we tell Flymake about our new diagnostic icons.

#+NAME: flymake-fringe-symbols
#+begin_src emacs-lisp
(put 'flymake-error 'flymake-bitmap
     (propertize "  " 'face `(:inherit (flymake-error default) :underline nil)))
(put 'flymake-warning 'flymake-bitmap
     (propertize "  " 'face `(:inherit (flymake-warning default) :underline nil)))
(put 'flymake-note 'flymake-bitmap
     (propertize "  " 'face `(:inherit (flymake-note default) :underline nil)))
#+end_src
...
Issues:
1. eldoc(+box) not working..
2. missing inline hints
