...
(use-package centaur-tabs
  ...
  :hook (dashboard-mode . centaur-tabs-mode)
  :config
...
1.XX+s startup time is caused by Emacs dashboard. Emacs startup time without it is < 0.8Xs.
...
(use-package shackle
  ...
  (shackle-rules
   `((help-mode                            :align right :select t :size 0.45)
     ...
     ("*mu4e-headers*"                     :align right :select t :size 0.78)
...
***** Dirvish: Polished Batteries For Dired
...
(use-feature mu4e
  ...
  (setq mu4e-contexts
        (list (make-mu4e-context
               :name "Disroot"
               :enter-func (lambda () (mu4e-message "Entering context disroot"))
               :leave-func (lambda () (mu4e-message "Exiting context disroot"))
               :match-func
               (lambda (msg) (when msg
                          (string-match-p "/icy-thought" (mu4e-message-field msg :maildir))))
               :vars '((user-full-name     . "Icy-Thought")
                       (user-mail-address  . "IcyThought@disroot.org")
                       (mu4e-drafts-folder . "/icy-thought/Drafts")
                       (mu4e-sent-folder   . "/icy-thought/Sent")
                       (mu4e-refile-folder . "/icy-thought/Archive")
                       (mu4e-trash-folder  . "/icy-thought/Trash")))))
  :custom
...
****** MU4E Dashboard: A Clean Interface For E-Mail's

#+begin_src emacs-lisp
(use-package mu4e-dashboard
  :ensure (:host github :repo "rougier/mu4e-dashboard")
  :after (mu4e)
  :hook ((mu4e-main-rendered . mu4e-dashboard)
         (mu4e-dashboard-mode . (lambda () (visual-fill-column-mode -1))))
  :config (setq mu4e-dashboard-file (expand-file-name "mu4e/sidebar.org" irkalla/underworld)))
#+end_src
...
(use-package yasnippet
  ...
  :hook (dashboard-mode . yas-global-mode)
  :config
