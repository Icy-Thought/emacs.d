...
(use-feature newsticker
  ...
  (defun irkalla/newsticker-quit-newTab ()
    ...
    (newsticker-stop)
    (tab-close))
  :hook (newsticker-treeview-item-mode . visual-fill-column-mode)
...
(use-feature telega ;; <-^ fetched from Nixpkgs
  :commands (telega)
  :hook (telega-chat-mode . visual-fill-column-mode)
  :custom
...
(use-package nix-ts-mode
  ...
  (defun eglot-nix-setup ()
    (with-eval-after-load 'eglot
      (let ((langserv "nil"))
        (when (executable-find langserv)
          (add-to-list 'eglot-server-programs '(nix-ts-mode . langserv)))))
    (eglot-ensure))
...
(use-package lua-mode
  ...
  (defun eglot-lua-setup ()
    (with-eval-after-load 'eglot
      (let ((langserv "lua-language-server"))
        (when (executable-find langserv)
          (add-to-list 'eglot-server-programs '(lua-mode . langserv)))))
    (eglot-ensure))
...
(use-package kotlin-mode
  :mode (("\\.kt\\'" . kotlin-mode)
         ("\\.kts\\'" . kotlin-mode))
  :preface
  (defun eglot-kotlin-setup ()
    (with-eval-after-load 'eglot
      (let ((langserv "kotlin-language-server"))
        (when (executable-find lsp)
          (add-to-list 'eglot-server-programs '(kotlin-mode . langserv)))))
    (eglot-ensure))
  :hook (lua-mode . eglot-kotlin-setup))
#+end_src
...
(use-package typst-ts-mode
  ...
  (defun eglot-typst-setup ()
    (with-eval-after-load 'eglot
      (let ((langserv "typst-lsp"))
        (when (executable-find langserv)
          (add-to-list 'eglot-server-programs '(typst-ts-mode . langserv)))))
    (eglot-ensure))
...
(use-package zig-mode
  ...
  (defun eglot-zig-setup ()
    (with-eval-after-load 'eglot
      (let ((langserv "zls"))
        (when (executable-find langserv)
          (add-to-list 'eglot-server-programs `((zig-mode zig-ts-mode) . langserv)))))
    (eglot-ensure))
