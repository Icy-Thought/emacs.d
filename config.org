...
(use-package kotlin-mode
  ...
  (defun eglot-kotlin-setup ()
    (with-eval-after-load 'eglot
      (let ((server "kotlin-language-server"))
        (when (executable-find lsp)
          (add-to-list 'eglot-server-programs `(kotlin-mode ,server)))))
    (eglot-ensure))
...
(use-package lua-mode
  ...
  (defun eglot-lua-setup ()
    (with-eval-after-load 'eglot
      (let ((server "lua-language-server"))
        (when (executable-find server)
          (add-to-list 'eglot-server-programs `(lua-mode ,server)))))
    (eglot-ensure))
...
(use-package nix-ts-mode
  ...
  (defun eglot-nix-setup ()
    (with-eval-after-load 'eglot
      (let ((server "nil"))
        (when (executable-find server)
          (add-to-list 'eglot-server-programs `(nix-ts-mode ,server)))))
    (eglot-ensure))
  ...
  (defun eglot-rust-setup ()
    (with-eval-after-load 'eglot
      (let ((server "rust-analyzer"))
        (when (executable-find server)
          (add-to-list 'eglot-server-programs `((rust-mode rust-ts-mode) ,server)))))
    (eglot-ensure))
  :hook (rust-mode . eglot-rust-setup)
...
(use-package typst-ts-mode
  ...
  (defun eglot-typst-setup ()
    (with-eval-after-load 'eglot
      (let ((server "typst-lsp"))
        (when (executable-find server)
          (add-to-list 'eglot-server-programs `(typst-ts-mode ,server))))
      (eglot-ensure)))
...
(use-package zig-mode
  ...
  (defun eglot-zig-setup ()
    (with-eval-after-load 'eglot
      (let ((server "zls"))
        (when (executable-find server)
          (add-to-list 'eglot-server-programs `((zig-mode zig-ts-mode) ,server)))))
    (eglot-ensure))
