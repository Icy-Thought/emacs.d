...
(use-package centaur-tabs
  ...
  :general (general-def 'normal centaur-tabs-mode-map
...
(use-package telephone-line
  ...
  (with-eval-after-load 'meow
    (telephone-line-meow-use-short-tag t))
...
(use-package eldoc-box
  :after '(eldoc eglot)
  :general (general-def 'normal prog-mode-map
...
(use-package marginalia
  ...
  :general (general-def 'normal minibuffer-local-map
...
**** EVIL: VIM bindings inside Emacs (disabled)
:PROPERTIES:
:header-args+: :tangle no
:END:
...
**** MEOW: Yet Another Modal Editing

Are you someone who is interested in Meow, but contemplating your decision over whether you should use it or not?
Then I recommend you take some time off to read the [[https://github.com/meow-edit/meow/blob/master/VIM_COMPARISON.org][vim comparison] page written by the author.

Being able to toggle =CaSeS= on command would save us a lot of time. Therefore adding it should be prioritized.

#+begin_src emacs-lisp
;;;###autoload
(defun irkalla/toggle-case ()
  "Highlighted region -> upper/lower case region. Otherwise, apply only to CHAR."
  (interactive)
  (let* ((beg (point))
         (end (1+ (point)))
...
Installing ~MEOW~ and notifying it about our pre-defined bindings.
...
(use-package meow
  ...
  (setq meow-char-thing-table
        '((?\( . round) (?\) . round) (?\[ . square) (?\] . square) (?{ . curly) (?} . curly) (?\" . string)
          (?r . tex-round) (?s . tex-square) (?c . tex-curly)
          (?„ . quote-gqq) (?‚ . quote-gq) (?\“ . quote-eqq) (?\‘ . quote-eq)
          (?s . symbol) (?. . sentence) (?b . buffer) (?p . paragraph) (?l . line)))
  ...
  ;; :NOTE| Pre-defined behaviour for our different modes
...
(use-package corfu
  ...
  :general (general-def 'insert corfu-map
            "TAB"   #'corfu-next
            [tab]   #'corfu-next
...
(use-package jinx
  ...
  :general
  (general-def 'normal (prog-mode-map text-mode-map)
...
(use-package ob
  ...
  :general (general-def 'normal org-mode-map
