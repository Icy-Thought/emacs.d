...
(use-package general
 ...
 (general-create-definer irkalla/backslash-lead-keydef
    :keymaps 'override
...
**** Hydra: A System for our Keybindings
...
Pulling Hydra with prettyfying techinques should be the default of Irkalla!
...
(use-package pretty-hydra
  ...
  :general (general-nmap "SPC" 'main-hydra/body)
  :config
...
;; :NOTE| Integrate ~pretty-hydra~ with ~use-package~!
(elpaca-wait)
#+end_src

***** (Hydra) General Emacs Bindings
...
(use-package emacs
  ...
  :config
  <<hydra-main-head>>
  <<hydra-general-actions>>
  <<hydra-option-toggles>>
  <<hydra-window-controls>>)
#+end_src

#+NAME: hydra-main-head
#+begin_src emacs-lisp :tangle no
(pretty-hydra-define main-hydra
  (:title (pretty-hydra-title "Main Hydra Head" 'sucicon "nf-custom-emacs")
   :color amaranth :quit-key ("q" "C-g"))

  ("Emacs"
   (("g" general-hydra-actions/body "General Emacs" :exit t)
    ("p" project-hydra/body "Projects" :exit t)
    ("t" toggle-options-hydra/body "Toggle Options" :exit t)
    ("w" window-control-hydra/body "Window Controls" :exit t))

   "Editor"
   (("e" eglot-hydra/body "Eglot (LSP)" :exit t))))

#+end_src

#+NAME: hydra-general-actions
#+begin_src emacs-lisp :tangle no
(pretty-hydra-define general-hydra-actions
  (:title (pretty-hydra-title "General Emacs" 'faicon "nf-seti-project") 
   :color amaranth :quit-key ("q" "C-g"))

  ("Finder"
   (("f f" find-file "Files in directory" :exit t))

   "Buffers"
   (("b d" kill-this-buffer "Exit active buffer" :exit t)
    ("b n" next-buffer      "Switch -> next buffer" :exit t)
    ("b p" previous-buffer  "Switch -> prev. buffer" :exit t)
    ("b s" scratch-buffer   "Switch -> curr. scratch-buf" :exit t))

   "Evaluation"
   (("e e" eval-expression "input" :exit t)
    ("e b" eval-buffer     "buffer" :exit t)
    ("e r" eval-region     "highlight" :exit t))

   "Emacs"
   (("q r" restart-emacs "Restart Emacs!" :exit t)
    ("q q" kill-emacs    "Quit Emacs..." :exit t))))
#+end_src

#+NAME: hydra-option-toggles
#+begin_src emacs-lisp :tangle no
(pretty-hydra-define toggle-options-hydra
  (:title (pretty-hydra-title "Options to Toggle" 'faicon "toggle-on")
   :color amaranth :quit-key ("q" "C-g"))
...
#+NAME: hydra-window-controls
#+begin_src emacs-lisp :tangle no
(pretty-hydra-define window-control-hydra
  (:title (pretty-hydra-title "Manage Emacs Windows" 'faicon "windows")
   :color amaranth :quit-key ("q" "C-g"))
...
(use-package project
  :elpaca nil
  :pretty-hydra
  ((:title (pretty-hydra-title "Project Management" 'faicon "nf-seti-project") 
    :color amaranth :quit-key ("q" "C-g"))
...
(use-package eglot
  :elpaca nil
  :pretty-hydra
  ((:title (pretty-hydra-title "Eglot (LSP)" 'faicon "nf-md-code_braces_box") 
    :color amaranth :quit-key ("q" "C-g"))

   ("Navigate"
    (("d" eglot-find-declaration "Find -> definition" :exit t)
     ("i" eglot-find-implementation "Find -> implementation" :exit t)
     ("D" eglot-find-typeDefinition "Find -> type-def" :exit t))

    "Edit"
    (("r" eglot-rename "Rename $SYMB -> New-Name" :exit t)
     ("a" eglot-code-actions "Perform Code-actions" :exit t))

    "Format"
    (("=" eglot-format-buffer "Format -> buffer" :exit t)
     ("]" eglot-format "Format -> region" :exit t))
...
(use-package expand-region
  :general
  (general-def 'visual (prog-mode-map text-mode-map)
    ")" 'er/expand-region
    "(" 'er/contract-region))
#+end_src
