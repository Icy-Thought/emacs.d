...
Make sure that the ability of ~Vertico~ is not limited due to certain pre-configured settings.

#+begin_src emacs-lisp
(use-package emacs
  :elpaca nil
  :custom
  ;; :NOTE| Enables searching outside of dir
  (file-name-shadow-mode 1)
  ;; :NOTE| Clears old-path when overridden -> new-path
