...
- [ ] (ORG-MODE) Rust babel <- :FIX| issue with main package...
- [ ] Completion + Tab behaviour.. (inser tab when no word prior to completion = desired)
- [ ] (ORG-MODE) Inline completion eglot.. :FIX| issue with current eglot implementation!)
- [ ] Fix background of Emacs dashboard logo + latex preview images! (:FIX| Emacs problem -> nothing to be fixed...)
